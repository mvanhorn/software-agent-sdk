...
class LLM(BaseModel, RetryMixin, NonNativeToolCallingMixin):
    ...
    def _get_litellm_api_key_value(self) -> str | None:
        ...
        if self.is_subscription:
            ...
            api_key_value = self._subscription_api_key_from_credentials(
                auth, credentials
            )
        elif self.api_key:
        ...
        return self._normalize_litellm_api_key_value(api_key_value)

    async def _aget_litellm_api_key_value(self) -> str | None:
        if not self.is_subscription:
            return self._get_litellm_api_key_value()
        ...
        if credentials is None:
            raise ValueError("OpenAI subscription login is required")
        return self._normalize_litellm_api_key_value(
            self._subscription_api_key_from_credentials(auth, credentials)
        )
    ...
    async def _atransport_call(
        ...
        **kwargs,
        ...
        api_key_value = await self._aget_litellm_api_key_value()
        with self._litellm_modify_params_ctx(self.modify_params):
            with warnings.catch_warnings():
                warnings.filterwarnings(
                    "ignore", category=DeprecationWarning, module="httpx.*"
